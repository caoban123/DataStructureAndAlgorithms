...
def is_perfect_util(root: Node, depth: int, level: int = 1) -> bool:
    if root is None:
        return True
    ...
    if root.left is None or root.right is None:
        return False
    return is_perfect_util(root.left, depth, level + 1) and is_perfect_util(root.right, depth, level + 1)
...
def print_special_nodes(root: Node) -> None:
    ...
    while queue:
        ...
        if node.left and node.right:
            if node.right.key % node.left.key == 0:
                result.append(node.key)
        if node.left:
            queue.append(node.left)
        if node.right:
            queue.append(node.right)
    print("Special nodes:", result)
...
level_order(root) 
print("\nIs full tree:", is_full(root))
print("Is complete tree:", is_complete(root))   
print("Is perfect tree:", is_perfect(root))
print_special_nodes(root)
print("Common ancestor of 5 and 9:", find_common_ancestor(root, 6, 5))